package com.hand.demo.infra.constant;

public class InvHeaderConstant {
    public static final String RULE_CODE = "DEMO-47358.INV_HEADER.INV_NUM";
    public static final String APPLY_TYPE_CODE = "DEMO-47358.INV_APPLY_HEADER.INV_TYPE";
    public static final String INVOICE_COLOR_CODE = "DEMO-47358.INV_APPLY_HEADER.INV_COLOR";
    public static final String APPLY_STATUS_CODE = "DEMO-47358.INV_APPLY_HEADER.APPLY_STATUS";
    public static final Integer TENANT_ID = 0;
}
